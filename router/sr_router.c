        ...
        else if (strncmp(interface, "eth2", 5) == 0)
            {
                ...
                if (node){
                    if (ip_type == ip_protocol_icmp) {
                        ...
                        struct sr_nat_mapping *mapping = sr_nat_lookup_external(&(sr->the_nat), icmp_header->identifier, nat_mapping_icmp);
                        if (!mapping){
                            return;
                        }
                        ...
                        icmp_header->identifier = mapping->aux_int;
                        icmp_header->icmp_sum = 0;
                        icmp_header->icmp_sum = cksum(icmp_header, len-sizeof(sr_ethernet_hdr_t)-sizeof(sr_ip_hdr_t));
                ...
                printf("Not directed to this router. Dropping packet ... \n");
                return;
            }
